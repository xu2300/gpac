...
typedef struct {
	...
	AES_KEY dec_key;
	unsigned char* block;
	unsigned char* previous_ciphertext;
	unsigned char* previous_cipher;
} Openssl_ctx_cbc;
...
GF_Err gf_crypt_set_state_openssl_cbc(GF_Crypt* td, const void *iv, int size)
{
	Openssl_ctx_cbc* ctx = (Openssl_ctx_cbc* )td->context;
	memcpy(ctx->previous_ciphertext, iv, size);
	memcpy(ctx->previous_cipher, iv, size);
	return GF_OK;
}

GF_Err gf_crypt_get_state_openssl_cbc(GF_Crypt* td, void *iv, int *size) {

	Openssl_ctx_cbc* ctx = (Openssl_ctx_cbc*)td->context;
	*size = td->algo_block_size;
...
GF_Err gf_crypt_encrypt_openssl_cbc(GF_Crypt* td, void *plaintext, int len) {
	Openssl_ctx_cbc* ctx = (Openssl_ctx_cbc*)td->context;
...
GF_Err gf_crypt_decrypt_openssl_cbc(GF_Crypt* td, void *ciphertext, int len) {
	Openssl_ctx_cbc* ctx = (Openssl_ctx_cbc*)td->context;
...
static GF_Err gf_crypt_set_state_openssl_ctr(GF_Crypt* td, const void *iv, int size) {
	...
	memcpy(ctx->c_counter, &((unsigned char*)iv)[1], size - 1);
	memcpy(ctx->enc_counter, &((unsigned char*)iv)[1], size - 1);
	return GF_OK;
}
